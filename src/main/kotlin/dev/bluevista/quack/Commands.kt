...
suspend fun registerCommands(kord: Kord, database: MongoDatabase) {
    ...
    kord.on<GuildChatInputCommandInteractionCreateEvent> {
        if (interaction.command.rootName == "mods") {
        ...
        } else if (interaction.command.rootName == "maps") {
            val response = interaction.deferPublicResponse()
            ...
            val result = collection.find(and(
                // Guns
            ...
            response.respond { content = count.toString() }
        }
    }
}